        ...
        if event_type & select.EPOLLIN:
            try:
                ...
                if data:
                    ...
                    unidate = json.loads(strdata).encode('utf-8')
                    jsondate = json.loads(unidate)
                    if jsondate is None:
                        return None
                    if jsondate.has_key("command") == False:
                        return None
                    if jsondate["command"] == "list_image":
                        list_image(c, jsondate["terminal_mac"])
            except Exception as e:
                print("except Exception as e:%s %s" % (str(e),data))
            finally:
                c.close()
...
def _run_server():
    ...
    # If not provided, event-mask defaults to (POLLIN | POLLPRI | POLLOUT). It can be modified later with modify().
    e.register(s.fileno())
